...
function getContentRankings(tab) {
    ...
    $("#" + tab).html("<div id='contentLoading'><img src='" + $FileURL + "/images/ajax.gif'></div>").fadeIn("very slow");

    $.ajax({

        url: $BaseURL + "/JSON/" + tab,
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if (json.errMsg != undefined) {
                ...
                $("#dataDiv").empty();
                $("#dataDiv").append('<div class="rankingContent"> <table id="dataTable" class="rankTable ' + pClass + '"><thead><tr>' + tableHeaders + '</tr></thead></table></div>');

                $('#dataTable').dataTable(json);
				   if (pURL.indexOf("UserRankings") > -1 || pURL.indexOf("ClanRankings") > -1) {
                    $(".top_ranking").show();

                    $.each(json.data, function (i, val) {
                        var UserID = $(val[1]).text();
                        var Points = $(val[4]).text();
                        if (i == 0) {
                            $("dl.top_ranking_no1 dt").html(UserID);
                            $("dl.top_ranking_no1 dd").html(Points);
                        }
                        if (i == 1) {

                            $("dl.top_ranking_no2 dt").html(UserID);
                            $("dl.top_ranking_no2 dd").html(Points);
                        }
                        if (i == 2) {
                            $("dl.top_ranking_no3 dt").html(UserID);
                            $("dl.top_ranking_no3 dd").html(Points);
                        }
                    });

                    $('table').children('tbody').children('tr:nth-child(1)').css("background-color", "#331414");
                    $('table').children('tbody').children('tr:nth-child(2)').css("background-color", "#363e48");
                    $('table').children('tbody').children('tr:nth-child(3)').css("background-color", "#3c2416");
                }
            }
        },
        "dataType": "json"
...
function LoadAll() {
    ...
    $(function () {
        ...
        if (URLPath.indexOf("index") > 0 || URLPath == "https://tr-ko.org/") {
            //updateLogonStatus();
            //updateAnnouncements();
        }
