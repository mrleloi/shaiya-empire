...
function updateLogonStatus() {
    ...
    $.get($BaseURL + '/serverstatus', function(data) {
...
function updateTeamspeakStatus() {
    ...
    WarcryQueue('onload').goNext();
    return;
    $.get("teamspeakStatus.html", function(data) {
        if (data == '1') {
            $this.addClass('online');
            $this.html('Online');
        } else {
...
function css_browser_selector(u) {
    ...
    var b = [(!(/opera|webtv/i.test(ua)) && /msie\s(\d)/.test(ua)) ? ('ie ie' + RegExp.$1) : is('firefox/2') ? g + ' ff2' : is('firefox/3.5') ? g + ' ff3 ff3_5' : is('firefox/3.6') ? g + ' ff3 ff3_6' : is('firefox/3') ? g + ' ff3' : is('gecko/index.html') ? g : is('opera') ? o + (/version\/(\d+)/.test(ua) ? ' ' + o + RegExp.$1 : (/opera(\s|\/)(\d+)/.test(ua) ? ' ' + o + RegExp.$2 : '')) : is('konqueror') ? 'konqueror' : is('blackberry') ? m + ' blackberry' : is('android') ? m + ' android' : is('chrome') ? w + ' chrome' : is('iron') ? w + ' iron' : is('applewebkit/index.html') ? w + ' ' + s + (/version\/(\d+)/.test(ua) ? ' ' + s + RegExp.$1 : '') : is('mozilla/index.html') ? g : '', is('j2me') ? m + ' j2me' : is('iphone') ? m + ' iphone' : is('ipod') ? m + ' ipod' : is('ipad') ? m + ' ipad' : is('mac') ? 'mac' : is('darwin') ? 'mac' : is('webtv') ? 'webtv' : is('win') ? 'win' + (is('windows nt 6.0') ? ' vista' : '') : is('freebsd') ? 'freebsd' : (is('x11') || is('linux')) ? 'linux' : '', 'js'];
    c = b.join(' ');
    h.className += ' ' + c;
    return c;
}
...
(function($) {
    ...
    var methods = {
            ...
            $element.find('option').each(function(index, element) {
                ...
                if (typeof $(element).attr('selected') != 'undefined') {
                    ...
                    if (typeof $(element).attr('disabled') != 'undefined') {
                        ...
                        $(option).bind('click', function(event) {
                            event.stopPropagation();
                            $element.SelectTransform('selectEvent', {
                                option: element,
                                index: index
                            });
                        });
                ...
                $(searchbox).on('keyup', function(event) {
                    ...
                    $(config.list).children('ul').each(function(index, element) {
                        ...
                        if (text == '' || text == null) {
                            ...
                            config.searchQueue = setTimeout(function() {
                                $element.SelectTransform('ScrollTo', 0);
                            }, 300);
                        } else if (thisString.indexOf(text) >= 0) {
                            clearTimeout(config.searchQueue);
                            config.searchQueue = setTimeout(function() {
                                $element.SelectTransform('ScrollTo', index);
                            }, 300);
                        }
                    });
                });
        ...
        selectEvent: function(options) {
            ...
            $(this).SelectTransform('selectOption', {
        ...
        ScrollUp: function() {
            ...
            $(config.list).stop(true, true).animate({
                marginTop: '-' + ($find.position().top) + 'px'
            }, 400);
...
(function($) {
    var methods = {
        ...
        state1: function(callback) {
            var $element = $(this);
            $element.find('#bar').stop(true, true).animate({
                width: '100px'
            }, 1000, function() {
...
(function($) {
    ...
    var methods = {
        ...
        open: function(html, buttons) {
            ...
            $('#Alert-box_container').stop().animate({
                opacity: 1
            }, 'fast');
...
var cookieconsent_options = {
    ...
    dismiss: "Got it!",
    learnMore: "More info",
    link: $BaseURL + "/index.php?page=cookie-policy",
    delay: 5000,
};
