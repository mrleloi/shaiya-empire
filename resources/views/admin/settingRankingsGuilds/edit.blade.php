...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.setting-rankings-guilds.update", [$settingRankingsGuild->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label for="title">{{ trans('cruds.settingRankingsGuild.fields.title') }}</label>
                    <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $settingRankingsGuild->title) }}">
                    @if($errors->has('title'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('header') ? 'is-invalid' : '' }}" type="text" name="header" id="header" value="{{ old('header', $settingRankingsGuild->header) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('num_display') ? 'is-invalid' : '' }}" type="number" name="num_display" id="num_display" value="{{ old('num_display', $settingRankingsGuild->num_display) }}" step="1">
