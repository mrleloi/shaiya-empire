...
//		$startIndex = ($criteria['pageSize'] * $criteria['page']) + 1;
//		$endIndex = $startIndex + $criteria['pageSize'];
		...
		$sql = "SELECT
				...
				FROM(
					...
					WHERE um.Status >= 0 AND um.Status != 16
        ...
        $results = [];
		try {
            $results = DB::connection('sqlsrv_gamedata')->select($sql);
        } catch (\Exception $e) {
            $results = [];
        }
    ...
    public function getCharacterKills($criteria = array()){
        // Map level filter to valid level ranges for PVP zones.
        $validLevels = array(
            0=>array(1,999),
            1=>array(1,15),
...
//		$endIndex = $startIndex + $criteria['pageSize'];
        ...
        $startDate = $today;
        $endDate = $today;
        if(isset($criteria['times']) && !empty($criteria['times'])){
            try {
                $startDate = Carbon::parse($criteria['start_date'])->toDateString();
                $endDate = Carbon::parse($criteria['end_date'])->toDateString();
            } catch (\Exception $e) {
                $startDate = $today;
                $endDate = $today;
            }
        }
        $times = "CAST(al.actiontime AS DATE) >= '". $startDate ."' AND CAST(al.actiontime AS DATE) <= '". $endDate ."'";
        ...
        $sql = "SELECT
                ...
                FROM
                 (SELECT
                    CAST(al.actiontime AS DATE) AS KillDate,
                    COUNT(*) AS Kills,
                    al.UserUID,
                    al.CharID
                    FROM
                        ps_gamelog.dbo.actionlog al
                        INNER JOIN PS_GameData.dbo.Chars c ON al.CharID = c.CharID AND al.UserUID = c.UserUID
                        INNER JOIN PS_UserData.dbo.Users_Master um ON um.UserUID = c.UserUID
                        INNER JOIN PS_GameData.dbo.UserMaxGrow umg ON umg.UserUID = um.UserUID
                        LEFT JOIN PS_GameData.dbo.GuildChars gc ON gc.CharID = c.CharID AND gc.Del = 0
                        LEFT JOIN PS_GameData.dbo.Guilds g ON g.GuildID = gc.GuildID
                    WHERE
                        um.Status >= 0 AND um.Status != 16
                        AND c.Level BETWEEN ".$minLevel." AND ".$maxLevel."
                        ".$classSQL."
                        ".$factionSQL."
                        AND c.Del = 0
                        AND al.actiontype = 103
                    GROUP BY
                        CAST(al.actiontime AS DATE), al.UserUID, al.CharID
                    HAVING
                        ". $times .") AS t1
                LEFT JOIN (". $sql .") AS t2
                ON t1.UserUID = t2.UserUID AND t1.CharID = t2.CharID
                ORDER BY
                    t1.Kills DESC;";
        $results = [];
        try {
    ...
    public function getGuildRanks($criteria = array()){
        // Create SQL for page ordering.
        if(isset($criteria['pageOrder']) && !empty($criteria['pageOrder']) && isset($criteria['pageDirection']) && !empty($criteria['pageDirection'])){
            if($criteria['pageOrder'] == 'KDR'){
                $criteria['pageOrder'] = 'CASE WHEN (c.K1 = 0 OR c.K2 = 0) THEN 0 ELSE CAST(c.K1/CAST(c.K2 AS decimal(18,2)) AS decimal(18,2)) END';
            ...
            $pageOrderSQL = "ORDER BY g.GuildPoint DESC";
        }

//		$startIndex = ($criteria['pageSize'] * $criteria['page']) + 1;
//		$endIndex = $startIndex + $criteria['pageSize'];

        $sql = "SELECT RowIndex, Faction, GuildName, Leader, TotalMember, Point, COUNT(*) OVER() AS TotalCount
                FROM
                (
                    SELECT
                    Row_Number() OVER(".$pageOrderSQL.") AS RowIndex,
                    ...
                    g.MasterName AS Leader,
                    g.TotalCount AS TotalMember,
                    g.GuildPoint AS Point
                    FROM PS_GameData.dbo.Guilds g
                    LEFT JOIN PS_GameData.dbo.GuildChars gc ON gc.Del = 0 AND g.GuildID = gc.GuildID
                    LEFT JOIN PS_GameData.dbo.Chars c ON c.CharID = gc.CharID
                    INNER JOIN PS_UserData.dbo.Users_Master um ON um.UserUID = c.UserUID AND um.Status >= 0 AND um.Status != 16 AND c.Del = 0
                    INNER JOIN PS_GameData.dbo.UserMaxGrow umg ON umg.UserUID = um.UserUID AND umg.ServerID = 1
                    GROUP BY g.GuildName, g.MasterName, g.TotalCount, g.GuildPoint, umg.Country
                ) AS t1;";
        $results = [];
        try {
            $results = DB::connection('sqlsrv_gamedata')->select($sql);
	...
	public function getCharacterStatData($charID){
		...
		$args = array($charID);
		$results = odbc_execute($stmt,$args);
